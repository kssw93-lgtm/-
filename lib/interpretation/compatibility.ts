...
export function describeElementRelation(elA: ElementId, elB: ElementId): { text: string; tier: ElementRelationTier } {
  ...
  // data/five-elements.json의 오행 생/극 표는 5개 오행이 완전한 순환 구조라 서로 다른
  // 두 오행은 항상 상생 아니면 상극 관계로 걸린다 — 지금 시점엔 이 분기가 실제로 도달되지
  // 않는다. 그래도 오행 데이터가 나중에 바뀌어 순환이 깨지는 경우를 대비한 방어적
  // 기본값으로 남겨둔다(생/극 어느 쪽에도 안 걸리는 조합이 생겨도 undefined 대신 안전한
  // 문구를 반환).
  return {
    tier: "neutral",
