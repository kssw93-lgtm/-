 ...
 */
function describeElementRelation(elA: ElementId, elB: ElementId): string {
  const labelA = ELEMENT_LABEL[elA];
  const labelB = ELEMENT_LABEL[elB];
  if (elA === elB) {
    return `두 사람의 일간은 같은 ${labelA} 기운이에요. 같은 파장을 공유해서 말하지 않아도 통하는 부분이 많지만, 같은 약점도 공유하니 서로의 단점을 보완해줄 제3의 존재(취미, 친구, 환경)를 곁에 두면 균형이 잘 잡혀요.`;
  }
  if (FIVE_ELEMENTS.generates[elA] === elB) {
    return `${labelA} 기운이 ${labelB} 기운을 북돋아주는 상생(相生) 관계예요. 한쪽이 자연스럽게 상대를 채워주고 힘을 실어주는 조합이라, 관계 안에서 서로 성장하는 느낌을 받기 쉬워요.`;
  }
  if (FIVE_ELEMENTS.generates[elB] === elA) {
    return `${labelB} 기운이 ${labelA} 기운을 북돋아주는 상생(相生) 관계예요. 방향은 다르지만 마찬가지로 서로를 채워주는 조합이라, 관계 안에서 서로 성장하는 느낌을 받기 쉬워요.`;
  }
  if (FIVE_ELEMENTS.controls[elA] === elB || FIVE_ELEMENTS.controls[elB] === elA) {
    return `${labelA}와(과) ${labelB}는 서로 견제하는 상극(相剋) 관계예요. 부딪히는 지점이 있을 수 있지만, 명리학에서는 적당한 상극이 오히려 서로를 긴장하게 하고 성장시키는 자극이 된다고도 봐요. 다름을 인정하고 조율하는 만큼 관계가 단단해지는 조합이에요.`;
  }
  return `${labelA}와(과) ${labelB}는 직접적인 생·극 관계는 없지만, 각자의 기운을 있는 그대로 존중해줄 때 무난하게 어울리는 조합이에요.`;
}
...
export function computeCompatibility(sajuA: SajuResult, sajuB: SajuResult): CompatibilityResult {
  ...
  const elementRelation = describeElementRelation(stemById(dayStemA).element, stemById(dayStemB).element);
